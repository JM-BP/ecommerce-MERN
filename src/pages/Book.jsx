    ...
    return bookData ? (
        <div className="border-t-2 pt-6 transition-opacity ease-in duration-500 opacity-100 max-w-6xl mx-auto">
            <div className="flex flex-col lg:flex-row gap-10 items-start">
                ...
                <div className="w-full lg:w-1/3 flex flex-col items-center">
                    ...
                    <div className="w-full max-w-xs">
                        <img className="w-full h-auto object-contain border border-gray-300" src={image} alt={bookData.name} />
                    </div>
                ...
                <div className="flex-1">
                    ...
                    <p className="text-gray-600 text-lg mt-2 font-semibold">{bookData.author}</p>
                    
                    {/* Ratings */}
                    <div className="flex items-center gap-1 mt-5">
                        {[...Array(3)].map((_, i) => <img key={i} src={assets.star_icon} alt="star icon" className="w-5" />)}
                        {[...Array(2)].map((_, i) => <img key={i} src={assets.star1_icon} alt="star icon" className="w-5" />)}
                    ...
                    <div className='flex flex-col gap-4 my-5'>
                        ...
                        <div className='flex gap-2'>
                            <button onClick={()=>setFormat("Book")} className={`border py-2 px-4 bg-gray-300 ${format === "Book" ? 'border-2 border-gray-600 font-semibold' : ''}`}>Book (Hardcover)</button>
                            <button onClick={()=>setFormat("eBook")} className={`border py-2 px-4 bg-gray-300 ${format === "eBook" ? 'border-2 border-gray-600 font-semibold' : ''}`}>eBook</button>
                        </div>
                    </div>
                    ...
                    {/* Add to Cart Button */}
                    <button onClick={()=>addToCart(bookData._id, format)} className="bg-black text-white px-8 py-3 text-lg mt-5 rounded-md hover:bg-blue-700">
                        Add to Cart
                    </button>
            ...
            <div className="mt-10 mb-5">
                ...
                <div className="flex">
                    <button 
                        className={`px-6 py-3 text-lg ${activeTab === 'description' ? 'border-b-2 border-gray-600 text-gray-700' : 'text-gray-400'}`} 
                        onClick={() => setActiveTab('description')}
                    >
                        Description
                    </button>
                    <button 
                        className={`px-6 py-3 text-lg ${activeTab === 'reviews' ? 'border-b-2 border-gray-600 text-gray-700' : 'text-gray-400'}`} 
