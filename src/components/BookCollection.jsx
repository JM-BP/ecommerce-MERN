...
const BookItem = ({id,image,name,genre,subgenre,price,tags})=> {
  ...
  return (
    <Link to={`/book/${id}`} className='text-gray-700 cursor-pointer p-2 pb-5  border border-gray-300' top={`/book/${id}`}>
        <div className='overflow-hidden'>
            <img className='hover:scale-110 transition ease-in-out h-80' src={image[0]} alt="" />
        </div>
